...
class Auger:
    ...
    output: dict
        Fitted Au coverage with standard error.
    """
    ...
    def fit_Au_coverage(
        ...
        -------
        string
            fitted Au coverage and standard error for each scan
        """
        ...
        for key, v in self.data.items():
            ...
            Au_coverage_err = alpha * (1 + beta) / \
                (1 + yita * alpha * rd ** 2)**2 * yita_err
            components = result.eval_components()
            self.output[key] = np.around(
                [Au_coverage, Au_coverage_err], decimals=3)
            ...
            if add_note is None:
                ...
                message = add_note + '\n'
            message += r'$I_{Au}:I_{Ni}$' + \
                ' = {:.3f} ± {:.3f}\n'.format(yita, yita_err)
            message += r'$\theta$' + \
                ' = {:.3f} ± {:.3f} ML'.format(Au_coverage, Au_coverage_err)
